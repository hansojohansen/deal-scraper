...
function DealBar({ value, max = 50 }: { value: number; max?: number }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-xs font-semibold text-green-700 w-10 text-right">-{value}%</span>
    </div>
  );
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        {stats.price_trend_30d.length > 1 && (
          <div className="bg-white rounded-xl border border-gray-200 p-4">
            <p className="text-sm font-medium text-gray-700 mb-3">Prisutvikling 30 dager</p>
        ...
        {stats.price_by_km_buckets.length > 0 && (
          <div className="bg-white rounded-xl border border-gray-200 p-4">
            <p className="text-sm font-medium text-gray-700 mb-3">Snittpris etter kilometerstand</p>
            <ResponsiveContainer width="100%" height={180}>
      ...
      {outliers && outliers.length > 0 && (
        <div>
          <h2 className="text-sm font-semibold text-gray-700 mb-3">Beste kjøp akkurat nå</h2>
              ...
              return (
                <a key={o.id} href={o.url} target="_blank" rel="noreferrer"
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-gray-900 truncate">{o.title ?? `${o.brand} ${o.model}`}</p>
                    <p className="text-xs text-gray-500">{o.year} · {o.mileage?.toLocaleString("no")} km</p>
                  </div>
                  <div className="w-32 shrink-0">
                    <DealBar value={d} />
                  </div>
                  <div className="text-right shrink-0 w-28">
                    <p className="font-bold text-gray-900 text-sm">{o.price?.toLocaleString("no")} kr</p>
                  </div>
                </a>
              );
            })}
